            ...
            $table.on('all.bs.table', function (e, name, args) {
                if (name == 'load-success.bs.table') {
                    for (var i = 0; i < ordersCompleted.length; i++) {
                        $("#table").find("[data-index='" + ordersCompleted[i] + "']").addClass('orderRowFinished');
                    }
                    ordersCompleted = [];
                }
            });
        ...
        function operateFormatter(value, row, index) {
            @if (Auth::user()->group_id == 1 ) 
                return [
                    ...
                    '<a class="edit" href="javascript:void(0)" title="{!!trans('messages.tooltip_send')!!}">',
                    '<i class="glyphicon glyphicon-send"></i>',
                    '</a> ',
                ].join('');
            @else {
                return [
                    '<a class="view" href="javascript:void(0)" title="{!!trans('messages.tooltip_stations_view')!!}">',
                    '<i class="glyphicon glyphicon-edit"></i>',
                ].join('');
            }
        }
